...
class Converter(customtkinter.CTk):
    def __init__(self):
        ...
        self.classes_list = ['. . .', KILO__METER(), METER__KILO(), KILO__MILE(), MILE__KILO(), MILE__METER(), METER__MILE(), KILO__GRAM(), GRAM__KILO(
        ), KILO__POUND(), POUND__KILO(), POUND__GRAM(), GRAM__POUND(), WATT__HP(), HP__WATT(), WATT__JOULEHOUR(), JOULEHOUR__WATT(), JOULEHOUR__HP(), HP__JOULEHOUR()]
        self.dict = {}
    ...
    def show_parents(self):
        ...
        self.parent_option = customtkinter.CTkOptionMenu(self, variable=self.parent_value, values=[
                                                         *self.parentsList], command=self.change_parent)
        self.parent_option.grid(row=0, column=1, pady=10, sticky='w')
        self.parent_option.configure(width=160, button_color=self.accent_color1,
                                     button_hover_color=self.accent_color1, dropdown_hover_color=self.accent_color2)
        ...
        for i in range(1, len(self.classes_list)):
            if self.parent_value.get() == self.classes_list[i].cat:
                ...
                if self.classes_list[i].child.split(' > ')[0] not in self.children_list1:
                    self.children_list1.append(
                        self.classes_list[i].child.split(' > ')[0])
                if self.classes_list[i].child.split(' > ')[1] not in self.children_list2:
                    self.children_list2.append(
                        self.classes_list[i].child.split(' > ')[1])
    ...
    def show_children(self):
        ...
        self.child_option1 = customtkinter.CTkOptionMenu(self, variable=self.child_value1, values=[
                                                         *self.children_list1], command=self.change_child)
        self.child_option1.grid(row=1, column=1, pady=10, sticky='w')
        self.child_option1.configure(width=160, button_color=self.accent_color1,
                                     button_hover_color=self.accent_color1, dropdown_hover_color=self.accent_color2)
        ...
        self.child_option2 = customtkinter.CTkOptionMenu(self, variable=self.child_value2, values=[
                                                         *self.children_list2], command=self.change_child)
        self.child_option2.grid(row=2, column=1, pady=10, sticky='w')
        self.child_option2.configure(width=160, button_color=self.accent_color1,
                                     button_hover_color=self.accent_color1, dropdown_hover_color=self.accent_color2)

        self.entry_label = customtkinter.CTkLabel(
            self, text=f'{self.child_value1.get()}', text_color=self.accent_color3)
        ...
        self.convert_button = customtkinter.CTkButton(
            self, text='Convert', width=160, height=35, cursor='hand2', command=self.convert)
        ...
        self.switch = customtkinter.CTkSwitch(
            self, text='', cursor='hand2', variable=self.switch_val, onvalue="on", offvalue="off", command=self.switch_child)
        ...
        self.output_label = customtkinter.CTkLabel(
            self, text=f'{self.child_value2.get()}', text_color=self.accent_color3)
        ...
        self.copy_button = customtkinter.CTkButton(
            self, text='Copy', width=70, height=35, cursor='hand2', command=self.copy)
        self.copy_button.grid(row=6, column=1, pady=20, sticky='e')
        self.copy_button.configure(hover_color=self.accent_color2)
        self.reset_button = customtkinter.CTkButton(
            self, text='Reset', width=70, height=35, cursor='hand2', command=self.reset)
        self.reset_button.grid(row=6, column=0, pady=20, sticky='s')
        self.reset_button.configure(
            fg_color=self.accent_color4, hover_color=self.accent_color5)
        self.copy_message = customtkinter.CTkLabel(
            self, text='', width=1, justify=RIGHT)
            ...
            if len(self.entry.get()) > 20:
                ...
                if key == self.child_value:
                    try:
                        ...
                        try:
                            self.result = float(
                                self.entry.get()) * self.dict[key]
                            self.output.configure(
                                text=str(round(self.result, 10)))
                            self.clip = str(round(self.result, 10))
                        except ValueError:
                            self.clean_child()
